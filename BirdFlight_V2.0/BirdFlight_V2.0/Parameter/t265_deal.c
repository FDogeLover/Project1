...
//												reference_accx,reference_accy,reference_accz; //参考加速度
	...
	if(rx.buf[0]==0x55 && rx.buf[1]==0xAA && rx.buf[63]==0xAA){	
		...
		if(rx.buf[2] == 0x10 &&  rx.buf[64] == CheckSum ){	
			...
			RT_Info.VioYaw   = - rpy_yaw.fvalue *180 /PI;    //当前的yaw
...
//			RotateZ = RotateScale *  Limits_data(  ((LPButterworth(RT_Info.GyroX,&RotateZ_filter_buf[0],&RotateY_Parameter)) * PI/180) ,4.0f,-4.0f);
			...
			if(rx.buf[43] == 0x20  && RT_Info.VioAbnormal == false  &&  FlightControl.landFlag==0)
			{
					...
					reference_yaw.cv[0] = rx.buf[57]; 
					reference_yaw.cv[1] = rx.buf[58];
					reference_yaw.cv[2] = rx.buf[59];
					reference_yaw.cv[3] = rx.buf[60];
					...
					Target_Info.Yaw =reference_yaw.fvalue;
...
void SendTakeOffFlag(void){
	...
	Uart5_tx(dataToARM,sizeof(dataToARM));
}
