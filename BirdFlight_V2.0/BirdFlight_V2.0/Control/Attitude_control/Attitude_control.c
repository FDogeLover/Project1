...
void Attitude_control(float PitchCalibration,float RollCalibration)
{
    ...
    {
        UAVThrust.PitchThrust = Limits_data( ( PID_Control(&PID_ParaInfo.PitchRate,&OriginalPitchRate,OriginalPitch.value,RT_Info.ratePitch,0.002f,400)) * Inertia_Wx , 1.3f ,-1.3f)
                                    //轴间耦合
    ...
    {
        UAVThrust.PitchThrust = Limits_data( (  PID_Control(&PID_ParaInfo.PitchRate,&OriginalPitchRate,Target_Info.RatePitch*0.0002f,RT_Info.ratePitch,0.002f,400)) * Inertia_Wx , 1.3f ,-1.3f)
                                                //轴间耦合
                                    + Limits_data(   RT_Info.rateRoll * (Inertia_Wz * RT_Info.rateYaw) - (Inertia_Wy * RT_Info.rateRoll) * RT_Info.rateYaw ,0.1f ,-0.1f)
                                                            //角速度前馈
                                                            + Limits_data( Inertia_Wx * GyroxFeedforwardLpf , 0.1f ,-0.1f);
    ...
    UAVThrust.PitchThrust =  Limits_data(UAVThrust.PitchThrust,1.4f,-1.4f);
    ...
    {
        UAVThrust.RollThrust =  Limits_data(  (  PID_Control(&PID_ParaInfo.RollRate,&OriginalRollRate,OriginalRoll.value,RT_Info.rateRoll,0.002f,400)) * Inertia_Wy, 1.3f ,-1.3f)
    ...
    {
        UAVThrust.RollThrust = Limits_data( (  PID_Control(&PID_ParaInfo.RollRate,&OriginalRollRate,Target_Info.RateRoll*0.0002f,RT_Info.rateRoll,0.002f,400)) * Inertia_Wy , 1.3f ,-1.3f)
                                    + Limits_data( (-(RT_Info.ratePitch * (Inertia_Wz * RT_Info.rateYaw) - (Inertia_Wx * RT_Info.ratePitch) * RT_Info.rateYaw)) ,0.1f ,-0.1f)
                                                            //角速度前馈
                                                            + Limits_data( Inertia_Wy * GyroyFeedforwardLpf , 0.1f ,-0.1f);
    }

    UAVThrust.RollThrust =  Limits_data(UAVThrust.RollThrust,1.4f,-1.4f);
    ...
    if(RockerControl.Navigation == 0)   //非手柄航向控制
    {
        if(YawHover ==1)
        {
...
void Calculate_Thrust(float arm_length){   //计算四个电机推力
    if(FlightControl.DroneMode==Drone_Mode_4Axis)    //四轴飞行
    {
    ...
    else if(FlightControl.DroneMode==Drone_Mode_Roll || FlightControl.DroneMode==Drone_Mode_RateRoll)
    {
		...
		UAVThrust.PaddleEffect = 1.0f; //要改
		
		/* 2023/7/19 LinYefong	失败版本↓2023/7/23
		 * 将电压变化近似成线性，以12V电压为基准分成电压高于12和低于12两种情况。*/
//		UAVThrust.PaddleEffect = (RT_Info.batteryVoltage < 12) * (1 - 0.1444 * fabs(12 - RT_Info.batteryVoltage)) + (RT_Info.batteryVoltage >= 12) * (1 + 0.13771 * fabs(12 - RT_Info.batteryVoltage));
...
void MotorThrust(float f1,float f2,float f3,float f4){
	...
	/* Tmotor 2213+8045的——>电机力转换油门百分比公式
...
//	M4 = (0.0028189f * f4 * f4 + 0.061661f * f4 + 0.32091f);
	...
	/*  2023/7/14 LinYeFong 改的
	 *  Tmotor 2213电机  +  T9545  力转换油门百分比公式
	 *  M1，M2，M3，M4为油门PWM占空比
	 *  f1，f2，f3，f4为四个电机所提供的力 */
	M1 = (0.0021569f * f1 * f1 + 0.065819f * f1 + 0.30872f);
	M2 = (0.0021569f * f2 * f2 + 0.065819f * f2 + 0.30872f);
	M3 = (0.0021569f * f3 * f3 + 0.065819f * f3 + 0.30872f);
	M4 = (0.0021569f * f4 * f4 + 0.065819f * f4 + 0.30872f);
